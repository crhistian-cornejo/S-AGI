                ...
                {isEditing ? (
                    ...
                    <>
                        ...
                        <span className="block text-[10px] text-muted-foreground/60">
                            {formatDate(file.updated_at)} · v{file.version_count}
                        </span>
                    </>
                )}
            </div>
...
function ScratchItem({ type, isSelected, hasDirtyContent, onSelect, onSaveAsNew }: ScratchItemProps) {
...
export function FilesSidebar({ type, onToggle }: FilesSidebarProps) {
    ...
    const [currentExcelFileId, setCurrentExcelFileId] = useAtom(currentExcelFileIdAtom)
    ...
    const currentFileId = type === 'excel' ? currentExcelFileId : currentDocFileId
    const setCurrentFileId = type === 'excel' ? setCurrentExcelFileId : setCurrentDocFileId
    const setCurrentFile = type === 'excel' ? setCurrentExcelFile : setCurrentDocFile
    ...
    const [newFileName, setNewFileName] = useState(type === 'excel' ? 'Nuevo archivo' : 'Nuevo documento')
    ...
    // Separate pinned and regular files
    const pinnedFiles = filteredFiles.filter((f) => f.is_pinned)
    const regularFiles = filteredFiles.filter((f) => !f.is_pinned)
    ...
    const handleConfirmSaveAsNew = useCallback(() => {
        ...
        createFileMutation.mutate({
            type,
            name: newFileName || (type === 'excel' ? 'Nuevo archivo' : 'Nuevo documento'),
            univerData: scratchData,
        })
        setSaveAsDialogOpen(false)
        setNewFileName(type === 'excel' ? 'Nuevo archivo' : 'Nuevo documento')
    }, [createFileMutation, fileSnapshotCache, scratchSessionId, newFileName, type])
    ...
    const handleSaveRename = useCallback((fileId: string, name: string) => {
        if (name.trim()) {
    ...
    const handleToggleArchive = useCallback((fileId: string) => {
            ...
            <Dialog open={saveAsDialogOpen} onOpenChange={setSaveAsDialogOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleConfirmSaveAsNew}>
                            Guardar
                        </Button>
                    </DialogFooter>
                </DialogContent>
