...
export const PdfBookmarksPanel = memo(function PdfBookmarksPanel({
    ...
    const handleNavigateToBookmark = useCallback((bookmark: PdfBookmark) => {
    ...
    }, [selectedPdf, newBookmarkTitle, newBookmarkNotes, currentPage, selectedText, setUserBookmarks])

    const handleDeleteBookmark = useCallback(async (bookmark: PdfBookmark) => {
    ...
    const openCreateDialog = useCallback(() => {
        ...
        if (selectedText?.text) {
            setNewBookmarkTitle(selectedText.text.slice(0, 50) + (selectedText.text.length > 50 ? '...' : ''))
        } else {
    ...
    return (
        <div className={cn("flex flex-col gap-2 p-2", className)}>
            ...
            <div className="flex items-center justify-between px-1">
                ...
                <Tooltip>
                    ...
                    <TooltipContent side="left">
                        {selectedText ? 'Bookmark selection' : 'Add bookmark at current page'}
                    </TooltipContent>
                </Tooltip>
            </div>
            ...
            {outline.length > 0 && (
                <Collapsible open={outlineOpen} onOpenChange={setOutlineOpen}>
                    <CollapsibleTrigger className="flex items-center gap-1 px-1 py-1 w-full hover:bg-muted/50 rounded text-xs font-medium">
                        {outlineOpen ? <IconChevronDown size={14} /> : <IconChevronRight size={14} />}
                        <IconBookmark size={14} className="text-blue-500" />
                        <span>Document Outline</span>
                        <span className="ml-auto text-muted-foreground">{outline.length}</span>
                    </CollapsibleTrigger>
                    <CollapsibleContent>
            ...
            <Collapsible open={userBookmarksOpen} onOpenChange={setUserBookmarksOpen}>
                <CollapsibleTrigger className="flex items-center gap-1 px-1 py-1 w-full hover:bg-muted/50 rounded text-xs font-medium">
                    {userBookmarksOpen ? <IconChevronDown size={14} /> : <IconChevronRight size={14} />}
                    <IconBookmarkFilled size={14} className="text-yellow-500" />
                    <span>My Bookmarks</span>
                    <span className="ml-auto text-muted-foreground">{userBookmarks.length}</span>
                </CollapsibleTrigger>
                <CollapsibleContent>
            ...
            <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
                <DialogContent className="sm:max-w-md">
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleCreateBookmark} disabled={!newBookmarkTitle.trim()}>
                            Create Bookmark
                        </Button>
                    </DialogFooter>
                </DialogContent>
        ...
        <div>
            ...
            >
                ...
                <span
                    ...
                    style={{ backgroundColor: bookmark.color || (bookmark.source === 'pdf' ? '#3B82F6' : '#FBBF24') }}
                />
                ...
                {onDelete && bookmark.source === 'user' && (
                    <button
                        ...
                        className="p-0.5 opacity-0 group-hover:opacity-100 hover:bg-destructive/20 hover:text-destructive rounded transition-all"
                    >
                        <IconTrash size={12} />
                    </button>
                )}
