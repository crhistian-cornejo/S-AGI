...
export const ChatInput = memo(function ChatInput({
  ...
  const [selectedDocument, setSelectedDocument] =
    useState<MentionableDocument | null>(null);
    ...
    (doc: MentionableDocument) => {
      ...
      if (mentionTriggerPosRef.current >= 0) {
        ...
        const afterQuery = value.slice(
          mentionTriggerPosRef.current + 1 + mentionSearchQuery.length,
        );
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Tab" && e.ctrlKey) {
      if (supportsResponseMode) {
        ...
        setResponseMode(
          (prev) =>
            ({ instant: "thinking", thinking: "auto", auto: "instant" })[
              prev
            ] as ResponseMode,
        );
  ...
  const activeWebSearch = useMemo(
    () => streamingWebSearches.find((ws) => ws.status === "searching"),
    [streamingWebSearches],
  );
  const latestWebSearchDone = useMemo(
    () =>
      [...streamingWebSearches].reverse().find((ws) => ws.status === "done"),
    [streamingWebSearches],
  );
  const activeFileSearch = useMemo(
    () => streamingFileSearches.find((fs) => fs.status === "searching"),
    [streamingFileSearches],
  );
  const latestFileSearchDone = useMemo(
    () =>
      [...streamingFileSearches].reverse().find((fs) => fs.status === "done"),
    [streamingFileSearches],
  );
  const activeTool = useMemo(
    () =>
      streamingToolCalls.find(
        (tc) => tc.status === "executing" || tc.status === "streaming",
      ),
    [streamingToolCalls],
  );
  ...
  return (
    ...
    <div
      ...
      className="relative flex flex-col gap-2 w-full max-w-[740px] mx-auto px-4 pb-4"
      ...
      >
        ...
        {images.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 px-2 pb-2">
            ...
            {compressionStats &&
              compressionStats.totalOriginal >
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        className={cn(
                          "h-8 w-8 text-muted-foreground/40 hover:text-foreground hover:bg-accent/50 rounded-xl relative",
                          (isMentionPopoverOpen || selectedDocument) &&
                          ...
                          if (textareaRef.current) {
                            ...
                            setTimeout(() => {
                              if (textareaRef.current) {
                                ...
                                textareaRef.current.focus();
                                // Trigger popover
                                mentionTriggerPosRef.current =
                                  cursorPos + (needsSpace ? 1 : 0);
                                setMentionSearchQuery("");
                                setMentionSelectedIndex(0);
                                setIsMentionPopoverOpen(true);
                              }
                    ...
                    <TooltipContent>
                      <p>
                        Mention document (@) - {mentionableDocuments.length}{" "}
                        docs available
                      </p>
                    </TooltipContent>
                  </Tooltip>
