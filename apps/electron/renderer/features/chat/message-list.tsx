...
export const MessageList = memo(function MessageList({
  ...
  return (
    <div className="space-y-6 px-4 py-4 max-w-[740px] mx-auto w-full overflow-hidden">
      ...
      {streamingError && <ErrorNotification error={streamingError} />}
...
const MessageItem = memo(function MessageItem({
  ...
  const contextWindow =
    message.metadata?.contextWindow ?? DEFAULT_CONTEXT_WINDOW;
  const hasUsage =
    totalTokens > 0 || (durationMs !== undefined && durationMs > 0);
      ...
      <div className="flex-1 min-w-0 space-y-2 pt-0.5 overflow-hidden">
        ...
        {content && (
          <div className="prose-container w-full overflow-hidden">
            ...
            {message.metadata?.documentCitations &&
              message.metadata.documentCitations.length > 0 && (
                <CitationsFooter
                  citations={message.metadata.documentCitations}
                />
              )}
          </div>
        )}
...
const AgentToolRenderer = memo(function AgentToolRenderer({
  ...
  if (toolType === "tool-WebSearch" || toolType === "tool-web_search") {
    return (
      <AgentWebSearch
        ...
        isNativeSearch={toolType === "tool-web_search"}
      />
    );
  }
  ...
  if (toolType === "tool-generate_image" || toolType === "tool-edit_image") {
    const prompt =
      typeof input.prompt === "string" ? input.prompt : "Generated image";
    const imageUrl =
      isRecord(output) && typeof output.imageUrl === "string"
        ? output.imageUrl
        : undefined;
  ...
  const artifactId =
    isRecord(toolCall.result) && "artifactId" in toolCall.result
      ? String(toolCall.result.artifactId)
      : null;
  const hasArtifact =
    artifactId &&
    (toolType === "tool-create_spreadsheet" ||
      toolType === "tool-create_document");
            ...
            <div className="flex items-center">
              ...
              {visibleFiles.map((file, index) => (
                ...
                >
                  <IconFile
                    size={10}
                    className={getFileExtensionColor(file.filename)}
                  />
                </div>
              ))}
              {/* URL favicons */}
              {visibleUrls.map((annotation, index) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      marginLeft:
                        index > 0 || visibleFiles.length > 0 ? "-4px" : 0,
...
const ViewChartsButton = memo(function ViewChartsButton({
  chartArtifactIds,
}: ViewChartsButtonProps) {
