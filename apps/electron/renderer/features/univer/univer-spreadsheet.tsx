...
export const UniverSpreadsheet = React.forwardRef<UniverSpreadsheetRef, UniverSpreadsheetProps>(({
    ...
    const saveArtifactSnapshot = trpc.artifacts.saveUniverSnapshot.useMutation({
        ...
        onError: (error, variables) => {
            ...
            console.error('[UniverSpreadsheet] Artifact save failed, keeping dirty flag:', failedId, error)
    ...
    const updateFileMutation = trpc.userFiles.update.useMutation({
        onSuccess: (result, variables) => {
            ...
            if (savedId) {
                ...
                setFileSnapshotCache(prev => {
                    const existing = prev[savedId]
                    if (existing) {
                        console.log('[UniverSpreadsheet] DB save confirmed, clearing dirty flag:', savedId)
                        return {
                            ...prev,
                            [savedId]: { ...existing, isDirty: false }
                        }
        ...
        onError: (error, variables) => {
            const failedId = variables.id
            console.error('[UniverSpreadsheet] DB save failed, keeping dirty flag:', failedId, error)
            if (failedId) {
            ...
            if (workbookRef.current && currentId) {
                try {
                    ...
                    if (snapshot) {
                        ...
                        console.log('[UniverSpreadsheet] Cached snapshot on unmount:', currentId, 'wasDirty:', wasDirty)
        ...
        if (workbookRef.current && oldId) {
            try {
                ...
                if (snapshot) {
                    ...
                    console.log('[UniverSpreadsheet] Cached snapshot before switch:', oldId, 'wasDirty:', wasDirty)
                    ...
                    if (wasDirty) {
                        ...
                        console.log('[UniverSpreadsheet] Triggered DB save before switch:', oldId)
                    }
                }
            } catch (err) {
