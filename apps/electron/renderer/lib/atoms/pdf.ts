...
export interface PdfNavigationRequest {
  ...
  /** Bounding box for precise text highlighting */
...
// === SOURCE HIGHLIGHTS (from agent citations) ===
...
export interface SourceHighlight {
  ...
  /** Highlighted text */
  text: string;
  /** Color for the highlight (derived from citation ID) */
  color: string;
}
...
/**
 * Active source highlights to show in the PDF viewer
 * Set when user clicks on a citation in the chat
 * Cleared when user dismisses or navigates away
 */
export const activeSourceHighlightsAtom = atom<SourceHighlight[]>([]);

/**
 * Add a source highlight
 */
export const addSourceHighlightAtom = atom(
  null,
  (get, set, highlight: SourceHighlight) => {
    const current = get(activeSourceHighlightsAtom);
    // Avoid duplicates by ID
    if (!current.some((h) => h.id === highlight.id)) {
      set(activeSourceHighlightsAtom, [...current, highlight]);
    }
  },
);

/**
 * Remove a source highlight by ID
 */
export const removeSourceHighlightAtom = atom(
  null,
  (get, set, highlightId: string) => {
    const current = get(activeSourceHighlightsAtom);
    set(
...
/**
 * Clear all source highlights
 */
export const clearSourceHighlightsAtom = atom(null, (_get, set) => {
  set(activeSourceHighlightsAtom, []);
});
 ...
 * Persisted to localStorage
 */
export const pdfChatPanelOpenAtom = atomWithStorage(
  "pdf-chat-panel-open",
  false,
);
 ...
 * Save status
 */
export const pdfSaveStatusAtom = atom<"idle" | "saving" | "saved" | "error">(
  "idle",
);
 ...
 */
export const addLocalPdfAtom = atom(null, (get, set, pdf: PdfSource) => {
 ...
 */
export function createPdfSourceFromChatFile(
  chatFile: {
    id: string;
    filename: string;
    storage_path: string;
...
export function createPdfSourceFromLocalFile(file: {
  ...
  const pathHash = btoa(
    encodeURIComponent(file.path).replace(/%([0-9A-F]{2})/g, (_, p1) =>
