...
export type ChangeType = 'created' | 'auto_save' | 'manual_save' | 'ai_edit' | 'ai_create' | 'restore' | 'import'
...
export function useUserFile(type: UserFileType) {
  ...
  const scheduleAutoSave = useCallback((data: { univerData?: unknown; content?: string }) => {
  ...
  // Toggle pin status
  const togglePin = useCallback(async (fileId: string) => {
    return togglePinMutation.mutateAsync({ id: fileId })
  }, [togglePinMutation])

  // Toggle archive status
  const toggleArchive = useCallback(async (fileId: string) => {
