...
export function useFileVersions(fileId: string | null) {
  ...
  const [previewVersion, setPreviewVersion] = useAtom(versionHistoryPreviewVersionAtom)
  ...
  const {
    ...
    { enabled: !!fileId && !!selectedVersions }
  )
  ...
  const openHistory = useCallback((fId: string) => {
  ...
  const selectVersionForPreview = useCallback((versionNumber: number | null) => {
    setPreviewVersion(versionNumber)
  }, [setPreviewVersion])

  // Select two versions for comparison
  const selectVersionsForComparison = useCallback((versionA: number, versionB: number) => {
  ...
  const fetchVersion = useCallback(async (versionNumber: number) => {
  ...
  const groupVersionsByDate = useCallback((versionsList: FileVersion[] | undefined) => {
