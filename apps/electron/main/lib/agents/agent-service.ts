...
export async function loadPDFContext(
  chatId: string,
  pdfPath: string,
): Promise<{ pages: PageContent[]; pdfBytes: Uint8Array } | null> {
  try {
    // Check cache first
    ...
    if (
      cached &&
      cached.path === pdfPath &&
      Date.now() - cached.loadedAt < PDF_CACHE_TTL_MS
    ) {
      log.info(`[AgentService] Using cached PDF context for ${chatId}`);
      return { pages: cached.pages, pdfBytes: cached.pdfBytes };
    }
    ...
    return { pages: result.pages, pdfBytes };
  } catch (error) {
    ...
    switch (selection.agent) {
      case "pdf": {
        if (!context.pdfPages || context.pdfPages.length === 0) {
          return {
            response:
              "No hay un PDF cargado para consultar. Por favor, sube un documento primero.",
            toolsUsed: [],
          };
        }
 ...
 */
export { OrchestratorAgent, PDFAgent, ExcelAgent, DocsAgent };
