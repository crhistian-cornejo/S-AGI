...
export async function processImage(
  ...
  if (originalSize > opts.maxInputSize) {
    throw new Error(
      `Image too large: ${formatBytes(originalSize)} exceeds limit of ${formatBytes(opts.maxInputSize)}`,
    );
  }
  ...
  try {
    ...
    log.info(
      `[ImageProcessor] Original: ${metadata.format}, ${metadata.width}x${metadata.height}, ${formatBytes(originalSize)}`,
    );
    ...
    log.info(
      `[ImageProcessor] Processed: ${opts.format}, ${processedMetadata.width}x${processedMetadata.height}, ${formatBytes(processedSize)} (${compressionRatio.toFixed(2)}x smaller)`,
    );
...
export async function processBase64Image(
  ...
  base64: string;
  mimeType: string;
  stats: Omit<ProcessedImage, "buffer">;
}> {
...
export async function processHeicImage(
  ...
  if (originalSize > MAX_HEIC_SIZE) {
    log.warn(
      `[ImageProcessor] HEIC file too large (${formatBytes(originalSize)}), skipping conversion`,
    );
    throw new Error(
      `HEIC file too large for conversion: ${formatBytes(originalSize)}`,
    );
  }

  log.info(
    `[ImageProcessor] Converting HEIC to JPEG: ${formatBytes(originalSize)}`,
  );
