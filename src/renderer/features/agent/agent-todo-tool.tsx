...
import { getTodosAtom, type TodoItem } from "@/lib/atoms"
import { IconCheck, IconArrowRight, IconCircle, IconChevronRight } from "@tabler/icons-react"

interface TodoToolPart extends ToolPart {
  toolCallId: string
  input?: {
    todos?: TodoItem[]
  }
...
function detectChanges(
  ...
  // Multiple changes - also show compact mode
...
const TodoStatusIcon = ({
  ...
  // During loading, show arrow for in_progress items
  ...
  switch (status) {
    case "completed":
      return (
        ...
        >
          <IconCheck className="w-2 h-2 text-muted-foreground" />
        </div>
      )
    case "in_progress":
      return (
...
export const AgentTodoTool = memo(function AgentTodoTool({
  ...
  // Synced todos state - scoped per subChatId
  const todosAtom = useMemo(
    () => getTodosAtom(subChatId || "default"),
    [subChatId],
  )
  ...
  useEffect(() => {
    if (newTodos.length > 0) {
      setSyncedTodos({ todos: newTodos, creationToolCallId: part.toolCallId })
    }
  ...
  // COMPACT MODE: Multiple updates - render summary
