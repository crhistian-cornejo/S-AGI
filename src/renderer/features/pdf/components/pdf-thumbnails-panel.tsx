...
export const PdfThumbnailsPanel = memo(function PdfThumbnailsPanel({
  ...
  const loadThumbnail = useCallback(
    async (pageIndex: number) => {
      ...
      // Mark as loading
      setThumbnails((prev) =>
        prev.map((t, i) => (i === pageIndex ? { ...t, loading: true } : t)),
      );
      ...
      try {
        ...
        console.log(
          `[PDF Thumbnails] Loaded thumbnail for page ${pageIndex + 1}`,
        );
      } catch (err) {
        console.error(
          `[PDF Thumbnails] Error loading page ${pageIndex + 1}:`,
          err,
        );
        setThumbnails((prev) =>
          prev.map((t, i) =>
  ...
  return (
    <div
      ref={containerRef}
      className={cn("flex flex-col gap-2 p-2 overflow-y-auto", className)}
    >
      {/* Header */}
      ...
      <div className="grid grid-cols-1 gap-2">
        {thumbnails.map((thumb) => (
          ...
          >
            {/* Thumbnail container */}
            <div className="relative bg-white rounded shadow-sm overflow-hidden w-full aspect-[3/4]">
              {thumb.blobUrl ? (
                <img
                  src={thumb.blobUrl}
