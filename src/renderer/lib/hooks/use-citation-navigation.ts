...
export function useCitationNavigation() {
  ...
  const setSourceHighlights = useSetAtom(activeSourceHighlightsAtom);
  ...
  const navigateToCitation = useCallback(
    (citation: CitationData) => {
      ...
      if (!isPdf) {
        ...
        console.log(
          "[CitationNavigation] Non-PDF citation clicked:",
          citation.filename,
        );
        return false;
      }

      // Create navigation request with bounding box if available
      ...
      // If we have bounding box data, create a source highlight
      if (
        ...
        const highlight = createSourceHighlight(
        ...
        // Clear existing highlights and set the new one
        setSourceHighlights([highlight]);
      }
    ...
    [currentChatId, setActiveTab, setNavigationRequest, setSourceHighlights],
  );
  ...
  const navigateToPage = useCallback(
    (filename: string, pageNumber: number, highlightText?: string) => {
      const request: PdfNavigationRequest = {
        filename,
        pageNumber,
        highlightText,
