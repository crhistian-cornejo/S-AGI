...
import { searchWithCitations, formatCitation, type PageContent } from './document-processor'
 ...
 */
export async function getDocumentContext(options: DocumentContextOptions): Promise<DocumentContext> {
    const { chatId, query, userId, searchContent = true, maxLength = MAX_CONTEXT_LENGTH } = options
...
async function searchDocumentsForContext(
    ...
    for (const doc of documents) {
        ...
        if (doc.pages && Array.isArray(doc.pages) && doc.pages.length > 0) {
            ...
            for (const result of results) {
                ...
                const citation = formatCitation(doc.filename, result.pageNumber, 'bracket')
        ...
        // Fallback: search in extracted_content without page numbers
            ...
            for (const word of queryWords) {
                ...
                if (index !== -1 && currentLength < maxLength) {
                    ...
                    const end = Math.min(doc.extracted_content.length, index + word.length + 200)
    ...
    const contextText = buildContextPrompt(contextParts, documents.map(d => d.filename), true)
 ...
 */
function getDocumentSummaries(documents: DocumentFile[], maxLength: number): DocumentContext {
    const contextParts: string[] = []
    let currentLength = 0
    ...
    for (const doc of documents) {
        ...
        if (summary) {
            ...
            const preview = doc.extracted_content.substring(0, MAX_DOC_PREVIEW_LENGTH)
    ...
    const contextText = buildContextPrompt(contextParts, documents.map(d => d.filename), false)
