...
export class CredentialManager {
    ...
    async setClaudeOAuth(credentials: ClaudeOAuthCredentials | null): Promise<void> {
        if (credentials) {
            await this.storage.set(
                { type: 'claude_oauth' },
                {
    ...
    async setChatGPTOAuth(credentials: ChatGPTOAuthCredentials | null): Promise<void> {
        if (credentials) {
            await this.storage.set(
                { type: 'chatgpt_oauth' },
                {
    ...
    async getTavilyKey(): Promise<string | null> {
        const cred = await this.storage.get({ type: 'tavily_api_key' })
        return cred?.value || null
    }

    async setTavilyKey(key: string | null): Promise<void> {
        if (key) {
            await this.storage.set(
                { type: 'tavily_api_key' },
                { value: key, metadata: { source: 'manual' } }
            )
            log.info('[CredentialManager] Tavily API key saved')
        } else {
            await this.storage.delete({ type: 'tavily_api_key' })
            log.info('[CredentialManager] Tavily API key removed')
        }
    }

    async hasTavilyKey(): Promise<boolean> {
        return this.storage.has({ type: 'tavily_api_key' })
    }
