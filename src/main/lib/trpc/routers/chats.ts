...
async function cleanupChatFiles(chatId: string, userId: string): Promise<void> {
  ...
  try {
    ...
    const { data: chatFiles, error: fetchError } = await supabase
      .from("chat_files")
      .select(
        "id, storage_path, openai_file_id, openai_vector_store_file_id, filename",
      )
      .eq("chat_id", chatId)
      .eq("user_id", userId);
    ...
    if (!chatFiles || chatFiles.length === 0) {
      ...
      if (storagePaths.length > 0) {
        ...
        if (deleteError) {
          log.error(
            "[Chats] Failed to delete files from storage:",
            deleteError,
          );
      ...
      if (dbDeleteError) {
        log.error(
          "[Chats] Failed to delete chat_files from DB:",
          dbDeleteError,
        );
      } else {
    ...
    if (messagesError) {
        ...
        if (msg.attachments && Array.isArray(msg.attachments)) {
          for (const attachment of msg.attachments) {
            if (
              ...
              attachment.storagePath.startsWith(`${userId}/`)
            ) {
      ...
      if (attachmentPaths.length > 0) {
        ...
        if (storageError) {
          log.error(
            "[Chats] Failed to delete message attachments from storage:",
            storageError,
          );
        } else {
          log.info(
            ...
            "message attachments from storage",
          );
        }
      }
      ...
      if (messagesDeleteError) {
        log.error(
          "[Chats] Failed to delete messages from DB:",
          messagesDeleteError,
        );
      } else {
...
> {
  ...
  for (const row of imagesRes.data || []) {
    try {
      const tc =
        (row.metadata as { tool_calls?: { name?: string }[] })?.tool_calls ||
        [];
      if (
        tc.some((t) => t?.name === "generate_image" || t?.name === "edit_image")
      )
        imageSet.add(row.chat_id);
    } catch {
      /* ignore */
    }
  }
...
export const chatsRouter = router({
      ...
      try {
        ...
        if (error) {
          ...
          if (
            ...
            log.warn(
              '[Chats] Column "pinned" not found, falling back to normal ordering. PLEASE RUN MIGRATION.',
            );
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      log.info(
        "[Chats] Creating chat with title:",
        input.title,
        "for user:",
        ctx.userId,
      );
      ...
      if (error) {
        if (
          ...
          log.warn(
            '[Chats] Column "pinned" not found during create, retrying without pinned. PLEASE RUN MIGRATION.',
          );
  ...
  // Get or create a document-specific chat (for PDF, Excel, Doc tabs)
  // This ensures each document has its own independent chat history
  getOrCreateDocumentChat: protectedProcedure
    .input(
      z.object({
        sourceType: z.enum(["pdf", "excel", "doc"]),
        sourceId: z.string().uuid(),
        title: z.string().optional(),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      const { sourceType, sourceId, title } = input;

      // First, try to find existing chat for this document
      ...
      if (findError) {
        log.error("[Chats] Error finding document chat:", findError);
        throw new Error(findError.message);
      }

      if (existing) {
        log.info(
          "[Chats] Found existing document chat:",
          existing.id,
          "for",
          sourceType,
          sourceId,
      ...
      const defaultTitle = title || `${sourceType.toUpperCase()} Chat`;
      ...
      if (createError) {
        log.error("[Chats] Error creating document chat:", createError);
        throw new Error(createError.message);
      }

      log.info(
        "[Chats] Created new document chat:",
        newChat.id,
        "for",
        sourceType,
        sourceId,
  ...
  listBySource: protectedProcedure
    .input(
      z.object({
        sourceType: z.enum(["pdf", "excel", "doc", "general"]),
      }),
    )
    .query(async ({ ctx, input }) => {
      try {
        ...
        if (error) {
          log.error("[Chats] List by source error:", error);
          throw new Error(error.message);
        }
        return enrichWithMeta(data || []);
      } catch (err) {
        log.error("[Chats] List by source exception:", err);
        return [];
      }
    }),
});
