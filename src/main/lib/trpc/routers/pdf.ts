        ...
        .query(async ({ ctx, input }) => {
            ...
            try {
                ...
                let artifactsQuery = supabase
                    .from('artifacts')
                    .select('id, chat_id, name, pdf_url, pdf_page_count, created_at, updated_at')
                    .eq('type', 'pdf')
                ...
                let filesQuery = supabase
                    .from('chat_files')
                    .select('id, chat_id, filename, storage_path, file_size, pages, metadata, processing_status, created_at')
                ...
                const { data: chatFiles, error: filesError } = await filesQuery
                    .range(offset, offset + limit - 1)
                ...
                const artifactPdfs: PdfSourceType[] = (artifacts || []).map(artifact => ({
        ...
        .query(async ({ ctx, input }) => {
            try {
                ...
                if (!file.pages || file.pages.length === 0) {
                    return { results: [], message: 'No extracted content available for search' }
                }
                ...
                for (const page of file.pages) {
                    ...
                    while (results.length < input.maxResults) {
                        ...
                        const end = Math.min(page.content.length, matchIndex + input.query.length + 100)
                        const context = page.content.slice(start, end)

                        results.push({
                            pageNumber: page.pageNumber,
                            content: (start > 0 ? '...' : '') + context + (end < page.content.length ? '...' : ''),
                            matchIndex
                        })
    ...
    findByFilename: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            try {
                ...
                let query = supabase
                    .from('chat_files')
                    .select('id, chat_id, filename, storage_path, pages, metadata, processing_status')
                    .eq('user_id', ctx.userId)
        ...
        .mutation(async ({ ctx, input }) => {
            try {
                ...
                if (input.sourceType === 'chat_file') {
                    ...
                    if (file.pages && file.pages.length > 0) {
                        ...
                        if (input.context?.currentPage) {
                            ...
                            documentContent = file.pages
                                ...
                                .map((p: { pageNumber: number; content: string }) =>
                                    `[Page ${p.pageNumber}]\n${p.content}`
                                )
                                .join('\n\n')
                        } else {
                    ...
                    documentContent = '(Content extraction not available for PDF artifacts)'
                }

                // Build the prompt for AI
                const selectedTextContext = input.context?.selectedText
                    ? `\n\nThe user has selected this text:\n"${input.context.selectedText}"`
                    : ''

                const systemPrompt = `You are a helpful assistant answering questions about a PDF document titled "${documentName}".
Use the document content provided to answer the user's question accurately.
